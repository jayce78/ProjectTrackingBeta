...
export default function App() {
  ...
  const durationSeries = useMemo(() => {
    ...
    return selected.tasks
      ...
      .map((t, idx) => ({ name: `${idx + 1}. ${t.title}`, ms: t.elapsedMs || 0 }));
  }, [selected]);
  ...
  const pauseTask = (taskId) => {
    ...
    updateTask(taskId, {
      status: "todo",
      activeStart: null,
      elapsedMs: (task.elapsedMs || 0) + Math.max(0, inc),
    });
  };
  ...
  const completeTask = (taskId) => {
    ...
    let inc = 0;
    if (task.activeStart) {
      inc = new Date().getTime() - new Date(task.activeStart).getTime();
    }
    updateTask(taskId, {
      status: "done",
      activeStart: null,
  ...
  // ---------- Stable handlers (prevents chart flicker) ----------
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 dark:bg-zinc-900 dark:text-zinc-100">
      ...
      <main className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 py-6 lg:grid-cols-12">
        ...
        <section className="lg:col-span-8 xl:col-span-9 space-y-6">
          ...
          <ProjectsOverview projects={projects} onSelect={handleSelectProject} />
          ...
          {!selected ? (
            ...
            <>
              <TaskSection
                ...
                newTaskDueLocal={newTaskDueLocal}
                setNewTaskDueLocal={setNewTaskDueLocal}
